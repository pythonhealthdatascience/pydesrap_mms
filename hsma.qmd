...
class Trial:
    ...
    def run_trial(self):
        ...
        for run in range(g.number_of_runs):
            ...
            self.df_trial_results.loc[run, 'scenario'] = [g.scenario_name]
...
my_trial = Trial()
df_trial_results_warmup = my_trial.run_trial()
...
for index, scenario_to_run in enumerate(all_scenarios_dicts):
    ...
    # Call the run trial method of our Trial object
    my_trial = Trial()
    results.append(my_trial.run_trial())
# View mean results by scenario
display(pd.concat(results).groupby('scenario').mean().head(20))
